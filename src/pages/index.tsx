...
export default function Home( {notes}: Notes ) {
  ...
  return (
    <>
     <div>
      <h1 className="text-center font-bold text-2xl mt-4">Dados</h1>
      <form onSubmit={e => {
        e.preventDefault()
        handleSubmit(form)
        ...
        <input type="text"
          ...
          className='border-2 rounded boredr-gray-600 p-1'
          />
          <textarea
          ...
          {notes.map(note => (
            <li key={note.id} className='border-b border-gray-600 p-2'>
              <div className="flex justify-between">
                ...
                <button onClick={() => deleteNote(note.id)} className="bg-red-500 px-3 text-white rounded" >Delete</button>
              </div>
            </li>
          ))}
        </ul>
